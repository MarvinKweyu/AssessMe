...
class StudentSignUpFormTest(TestCase):
    ...
    def test_if_user_subjects_are_being_saved(self):
        ...
        self.assertEqual(student.interests.first(), subject)


class BaseAnswerInlineFormSetTest(TestCase):
    def test_if_there_is_at_least_one_correct_answer(self):

        user = models.User.objects.create(is_teacher=True, username="Teacher Vitas")

        subject = models.Subject.objects.create(name="Vitas Career")
        ...
        question = models.Question.objects.create(quiz=quiz, text=question_text)
